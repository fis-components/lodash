...
(function () {
   ...
   */
     ...
     */
        function createComposer(fromRight) {
            return function () {
                var length = arguments.length, index = length, fromIndex = fromRight ? length - 1 : 0;
                if (!length) {
                    return function () {
                        return arguments[0];
                    };
 ...
 : object == 0 ? 1 / object == 1 / other : object == +other;
     ...
     */
        function difference() {
            var args = arguments, index = -1, length = args.length;
            while (++index < length) {
     ...
     */
        function sortByAll(collection) {
            ...
            var args = arguments, guard = args[3];
            if (guard && isIterateeCall(args[1], args[2], guard)) {
                args = [
                    collection,
                    args[1]
     ...
     */
        function attempt() {
            var func = arguments[0], length = arguments.length, args = Array(length ? length - 1 : 0);
            while (--length > 0) {
     ...
     */
        function sum(collection) {
            ...
            var length = collection.length, result = 0;
            while (length--) {
     ...
     */
        ...
        baseForOwn(LazyWrapper.prototype, function (func, methodName) {
            var lodashFunc = lodash[methodName], checkIteratee = /^(?:filter|map|reject)|While$/.test(methodName), retUnwrapped = /^(?:first|last)$/.test(methodName);
